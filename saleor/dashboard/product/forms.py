...
class ProductVariantForm(forms.ModelForm, AttributesMixin):
    ...
    class Meta:
        ...
        fields = [
            'sku', 'price_override',
            'quantity', 'cost_price', 'track_inventory', 'product_template']
        labels = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.product_template:
            self.fields['product_template'].widget = forms.HiddenInput({'value': json.dumps(self.instance.product_template, ensure_ascii=False)})
        else:
            self.fields['product_template'].widget = forms.HiddenInput()

    def clean_jsonfield(self):
        jdata = self.cleaned_data['product_template']
        try:
    ...
    def save(self, commit=True):
        ...
        self.instance.product_template = self.clean_jsonfield()
